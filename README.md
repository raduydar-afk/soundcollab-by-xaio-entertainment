...
Україномовний публічна GitHub Pages версія платформи для музичної індустрії та суміжних event-професіоналів. Сайт показує напрямок продукту, ролі клієнта й спеціаліста, пошук/запит, маркетплейс, order workspace, журнал, Академію та модель монетизації.
...
- Воркспейс і кабінети описані як продуктова структура: задачі, файли, статус, дедлайн і order-scoped chat.
...
Заявки та реєстрація спеціалістів зараз приймаються через робочу пошту. Онлайн-оплату свідомо відкладено до підключення платіжного сервісу; карткові дані на сайті не вводяться. Автентифікація, приватні файли, чат і календар підключатимуться поетапно.

## Архітектура та production-план

Поточний шар — публічний статичний сайт SoundCollab, який можна опублікувати з `main`. Наступний шар має винести UI на API з role-based доступом:
